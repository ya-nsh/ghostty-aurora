// aurora-deep-winter.glsl - generated from src/aurora.template.glsl.
// Variant: Deep Winter
//
// Ghostty setup:
//   custom-shader = /path/to/ghostty-aurora/aurora-deep-winter.glsl
...
const float TEXT_PROTECT = 0.94;

const int RIBBON_LAYERS = 3;
const int FBM_OCTAVES = 3;
const float CURTAIN_STRENGTH = 0.52;
const float HAZE_STRENGTH = 0.030;
const float STAR_INTENSITY = 0.0;
const float NORTH_STAR_INTENSITY = 0.0;

const vec3 MORNING_A = vec3(0.60, 0.86, 0.96);
const vec3 MORNING_B = vec3(0.48, 0.58, 0.88);
const vec3 EVENING_A = vec3(0.12, 0.64, 0.68);
const vec3 EVENING_B = vec3(0.24, 0.28, 0.78);
const vec3 NIGHT_A = vec3(0.02, 0.26, 0.30);
const vec3 NIGHT_B = vec3(0.03, 0.06, 0.22);
